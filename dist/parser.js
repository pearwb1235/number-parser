  ...
  (0, _createClass2["default"])(ParserConfig, [
    {
      ...
      value: function loadConfig(config) {
        ...
        var numberRegStr = Object.keys(this.config.number)
          .join("")
          .replace(/\./, "\\");
        var unitRegStr = Object.keys(this.config.unit)
          .join("")
          .replace(/\./, "\\");
        ...
        this.regexp = new RegExp(
          "[".concat(numberRegStr + unitRegStr + dotRegStr, "]")
        );
      ...
      value: function vaild(str) {
        ...
        for (var i = 0; i < str.length; i++) {
          if (this.config.dot.includes(str[i])) {
            if (dotFlag)
              throw new _error["default"](2, "小數點出現兩個(含)以上");
            dotFlag = true;
    ...
    },
    {
      ...
      value: function parseNumber(str) {
        ...
        if (result.toString() !== newStr)
          throw new _error["default"](999, "確認數字轉換結果失敗");
        return result;
      },
    },
  ]);
